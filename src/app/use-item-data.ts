...
const useItemData = () => {
    ...
    action: () => {
      dispatch(
        showActionOnPokemon((index: number) => {
          if (status !== null && pokemon[index].status !== status) return;
          if (status === null && !pokemon[index].status) return;
          dispatch(
            updateSpecificPokemon({
              index,
              pokemon: {
  ...
  const data: Record<string, ItemData> = {
    ...
    [ItemType.ParlyzHeal]: statusCure(
      ItemType.ParlyzHeal,
      "Parlyz Heal",
      "paralysis",
      200,
      ...
      action: () => {
        dispatch(
          showActionOnPokemon((index: number) => {
            ...
            dispatch(consumeItem(ItemType.FullRestore));
          })
        );
      },
    },
