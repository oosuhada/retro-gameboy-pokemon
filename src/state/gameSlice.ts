...
export const gameSlice = createSlice({
  ...
  reducers: {
    ...
    load: (state) => {
      ...
      state.seenPokemon = savedGameState.seenPokemon || Array.from(
        new Set([
          ...savedGameState.pokemon.map((pokemon) => pokemon.id),
          ...savedGameState.pc.map((pokemon) => pokemon.id),
        ])
      );
    },
    ...
    encounterPokemon: (state, action: PayloadAction<PokemonEncounterType>) => {
      ...
      if (!state.seenPokemon.includes(action.payload.id)) {
        state.seenPokemon.push(action.payload.id);
      }
    },
    endEncounter: (state) => {
      state.pokemonEncounter = undefined;
      state.pokemon.forEach((pokemon) => {
        pokemon.battle = undefined;
      });
    },
    setActivePokemon: (state, action: PayloadAction<number>) => {
      state.pokemon[state.activePokemonIndex].battle = undefined;
      state.pokemon[action.payload].battle = undefined;
    ...
    healPokemon: (state) => {
      ...
      for (let i = 0; i < state.pokemon.length; i++) {
        ...
        state.pokemon[i].status = undefined;
        state.pokemon[i].statusTurns = undefined;
        state.pokemon[i].battle = undefined;
      }
    },
