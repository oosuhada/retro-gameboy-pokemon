...
import resolveBattleTurn, { TurnResolution } from "../domain/red-blue/resolve-turn";
import {
  applyEndTurnEffects,
  getEffectiveSpeed,
  getForcedChargingMove,
  getStatusLabel,
} from "../domain/red-blue/battle-rules";
...
const PokemonEncounter = () => {
      ...
      dispatch(
        addPokemon({
          ...
          moves: enemy.moves.map((move) => {
            return {
              id: move,
              pp: getGen1MoveMetadata(getMoveMetadata(move)).pp || 0,
            };
          }),
          hp: enemyStats.hp,
        })
  ...
  const getRandomEnemyMove = () => {
    const chargingMove = getForcedChargingMove(enemy);
    if (chargingMove) return chargingMove;
  ...
  const getActiveMovesFirst = (
    ...
    const activeSpeed = getEffectiveSpeed(active);
    const enemySpeed = getEffectiveSpeed(enemy);
    if (activeSpeed === enemySpeed) return Math.random() < 0.5;
    return activeSpeed > enemySpeed;
  };

  const presentTurnResolution = (
        ...
        if (blocked) {
          setAlertText(
            `${activeMetadata.name.toUpperCase()} ${
              effectMessages[0] || "can't move!"
            }`
          );
        } else if (failed) {
          setAlertText(effectMessages[0] || `But it failed!`);
        } else if (missed) {
    ...
    if (!isAttacking) {
      ...
      setTimeout(() => {
        ...
        if (blocked) {
          setAlertText(
            `Enemy ${enemyMetadata.name.toUpperCase()} ${
              effectMessages[0] || "can't move!"
            }`
          );
        } else if (failed) {
  ...
  const finishTurn = (us: PokemonInstance, them: PokemonEncounterType) => {
    const endTurn = applyEndTurnEffects(us, them);
    dispatch(updatePokemon(endTurn.us));
    dispatch(updatePokemonEncounter(endTurn.them));

    if (endTurn.messages.length > 0) {
      setAlertText(endTurn.messages[0]);
      setTimeout(() => setAlertText(null), 900);
    }

    if (endTurn.them.hp <= 0) {
      setStage(20);
      return;
    }
    if (endTurn.us.hp <= 0) {
  ...
  const processBattle = (attackId: string) => {
    const resolvedAttackId = getForcedChargingMove(active) || attackId;
    const activeMove = getGen1MoveMetadata(getMoveMetadata(resolvedAttackId));
    const enemyMove = getGen1MoveMetadata(getMoveMetadata(getRandomEnemyMove()));
    ...
    if (activeMovesFirst) {
      // We are attacking
      const { us, them } = presentTurnResolution(
        resolveBattleTurn(active, enemy, resolvedAttackId, true),
        true
      );
      ...
      setTimeout(() => {
        ...
        // Enemy attacking
          ...
          const { us: usNew, them: themNew } = presentTurnResolution(
            resolveBattleTurn(us, them, enemyMove.id, false),
            false
          );
    ...
    // Enemy moving first
      ...
      // Enemy attacking
      const { us, them } = presentTurnResolution(
        resolveBattleTurn(active, enemy, enemyMove.id, false),
        false
      );
      ...
      setTimeout(() => {
        ...
        // We are attacking
          ...
          const { us: usAfterAttack, them: themAfterAttack } = presentTurnResolution(
            resolveBattleTurn(us, them, resolvedAttackId, true),
            true
          );
          ...
          setTimeout(() => {
            ...
            // Ending battle
              ...
              finishTurn(usAfterAttack, themAfterAttack);
            }
          }, ATTACK_ANIMATION + 1000);
        }
      }, ATTACK_ANIMATION + 1000);
          ...
          <StyledPokemonEncounter>
            ...
            >
              ...
              >
                ...
                <Level>{`:L${enemy.level}${enemy.status ? ` ${getStatusLabel(enemy.status)}` : ""}`}</Level>
                <HealthBarContainer>
                  <HealthBar
                    big
            ...
            >
              ...
              >
                ...
                <Level>{`:L${active.level}${active.status ? ` ${getStatusLabel(active.status)}` : ""}`}</Level>
                <HealthBarContainer>
                  <HealthBar
                    big
